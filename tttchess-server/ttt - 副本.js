exports.TTTChess = function()
{
	...
	var m_Rooms = [];//房间管理
	var m_RoomData = {};//房间内棋盘信息，wangjia 20160908
	var n_Clients = 0;
	var self = this;
	var io;//socket.io
	...
	//初始化棋盘数据 3*3*9格棋盘
	//wangjia 20160910 要是用对象修改，需要设计能保存全局棋盘的对象
	var InitChessData = function(roomIdx){
		m_RoomData.roomId = roomIdx;
		for(var i = 0; i < 9; i++){ //初始化9
			m_RoomData[roomIdx][i] = [];
			for(var j = 0; j < 3; j++){
				m_RoomData[roomIdx][i][j] = [];
				for(var k = 0; k < 3; k++)
					m_RoomData[roomIdx][i][j][k] = 0;
			}
		}
	}
